...
The sharp edges we hit building and validating roku-test, each with its root cause. Most are already
...
**Fix (already in roku-test):** the generated bsconfig references the plugin by **absolute path**, so
...
- **Interpreter can't parse the Rooibos runtime.** The lightweight `@rokucommunity/brs` interpreter errors
  with `Expected property name after '.'` on Rooibos's own `.brs`. roku-test uses **`brs-node`**, whose
  parser handles it.
- **Coverage-on build crashes headless.** With coverage enabled, instrumented code calls an on-device
  ...
  roku-test builds the headless lane with **coverage off**.
...
LCOV to the device console; it never writes a file. roku-test's `--lcov`:
...
- **`@rokucommunity/brs`** — lightweight, fast; good for pure logic. **Does not** implement crypto and
  **cannot parse the Rooibos runtime**. Not used by roku-test's Rooibos lane.
- **`brs-node`** (`brs-cli`) — fuller component set incl. crypto; parses the Rooibos runtime. This is the
  headless interpreter roku-test uses. Its **SceneGraph support is experimental**, so it does *not* run
  the scene-based Rooibos runner — which is why roku-test ships its own headless driver.
...
**Fix (already in roku-test):** the generated device bsconfig sets `"autoImportComponentScript": true`. If
...
even on hello-world under Node 22. It's unmaintained — use `brs-node` (what roku-test depends on).
