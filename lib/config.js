...
function findNodeSpecs(cfg) {
  const out = [];
  const skip = new Set(['node_modules', '.roku-test', '.git', 'out']);
  (function walk(dir) {
    let entries = [];
...
function writeBsConfig(cfg, lane, extra = {}) {
  ...
  // Global-context seeding: `globalFields` (a map of scene-field -> value in roku-test.json) is
  ...
  const bsconfig = {
    ...
    // BS1107 warns about each one — always silenced. Projects can add more codes via roku-test.json's
    ...
    plugins: [resolvePackageMain('@ramon-lobo/rooibos-roku')],
    rooibos,
  };
  fs.mkdirSync(staging, { recursive: true });
