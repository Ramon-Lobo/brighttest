...
const ONCHANGE = {
  ...
  replace: 'notifyObservers(){y.flushingNotifications&&this.lastNotifiedBatchId===y.currentBatchId||(y.flushingNotifications?this.dispatchObservers():(y.queuedFields.has(this)||(y.notifyQueue.push(this),y.queuedFields.add(this)),y.flushNotificationQueue()))}',
  appliedMarker: 'y.flushingNotifications?this.dispatchObservers()',
...
function installTtsBundle(pkgDir, version) {
  ...
  if (cur.includes('roTextToSpeech')) {
    console.log('[roku-test] brs-node roTextToSpeech bundle already installed');
    return;
  }
...
function installSgBundle(pkgDir, version) {
  ...
  let cur = '';
  try { cur = fs.readFileSync(target, 'utf8'); } catch (e) { /* will copy */ }
  if (cur.includes('DynamicPinPad')) {
    console.log('[roku-test] brs-node SceneGraph bundle (DynamicPinPad) already installed');
    return;
  }
  fs.copyFileSync(vendored, target);
  console.log(`[roku-test] installed rebuilt brs-node ${version} SceneGraph bundle (adds DynamicPinPad)`);
}
