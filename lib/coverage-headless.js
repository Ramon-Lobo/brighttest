...
function report(out, cfg, opts, nodeSpecs, killReason, coverage = true) {
  ...
  if (killReason === 'timeout') {
    console.error(`  brighttest (${label}): TIMED OUT after ${opts.timeout || 900}s`);
    console.log('='.repeat(56));
    return 1;
  }
  if (!sawResult) {
    console.error(`  brighttest (${label}): no result — the run did not complete`);
    console.log('='.repeat(56));
    return 1;
  }
  const nodeNote = nodeSpecs.length ? `, ${nodeSpecs.length} @SGNode suite(s) run headless` : '';
  const covNote = covM ? `  |  coverage ${covM[1]}%` : '';
  console.log(`  brighttest (${label}): ${passed} passed, ${failed} failed${nodeNote}${covNote}`);
