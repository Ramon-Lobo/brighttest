...
function run(cfg, opts) {
  ...
  if (!wantLcov) {
    ...
    if (timedOut(res)) {
      console.error(`\n[brighttest] device run timed out after ${opts.timeout || 900}s and was killed. ` +
  ...
  if (text) {
    ...
    console.log(`\n[brighttest] LCOV written: ${opts.lcov}  (${count} file record(s))`);
  } else {
    console.error('\n[brighttest] --lcov requested but no LCOV output found in the device run.');
