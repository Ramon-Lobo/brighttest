...
function render() {
  ...
  for (const n of nodes) {
    ...
    if (n.path === pinned) r.classList.add('hot');
...
function showDetail(n) {
  ...
  if (n.text) asserts.push(`- assertText: { ${sel.slice(2, -2)}, equals: ${JSON.stringify(n.text)} }`);
  ...
  $('#detail').innerHTML =
    ...
    `<div class="assert"><div class="k">suggested assertions</div>${asserts.map((a) => `<code>${escapeHtml(a)}</code>`).join('')}</div>`;
}
