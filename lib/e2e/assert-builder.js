...
const KINDS = ['visible', 'text', 'focused', 'gone', 'field'];
...
// selector. `text` reads the node's current visible text as the expected value; `field` (opts.field)
// reads the node's current value of that field. Throws on an unusable kind or a missing/absent field.
function buildAssertion(kind, node, all, opts = {}) {
  ...
  switch (kind) {
    ...
    case 'field': {
      const f = opts.field;
      if (!f) throw new Error('assertField needs a field name (--field <name>)');
      const raw = node.attrs ? node.attrs[f] : undefined;
      if (raw === undefined) throw new Error(`node has no field "${f}" — run inspect to see available fields`);
      return `- assertField: ${serializeInlineMap({ ...selector, field: f, equals: raw })}`;
    }
    default:
