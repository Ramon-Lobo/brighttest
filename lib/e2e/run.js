...
// Comma-split a --host value (or ROKU_HOST) into a de-duped list of device hosts.
function parseHosts(opts) {
  const raw = opts.host || process.env.ROKU_HOST || '';
  return [...new Set(String(raw).split(',').map((h) => h.trim()).filter(Boolean))];
}
...
async function run(cfg, opts) {
  ...
  const hosts = parseHosts(opts);
  const password = opts.password || process.env.ROKU_PASSWORD || null;
  if (!hosts.length) {
    process.stderr.write(c.red('[brighttest e2e] a device is required: --host <ip[,ip…]> (or ROKU_HOST). Dev --password enables screenshots.') + '\n');
    return 2;
  }

  // inspect/record are inherently single-device — use the first host.
  if (opts.e2eAction === 'inspect' || opts.e2eAction === 'record') {
    const device = createDevice({ host: hosts[0], password });
    let info;
    try { info = await device.deviceInfo(); }
  ...
  if (mode !== 'off' && !password) {
    process.stdout.write(c.yellow(`  note: --screenshots-mode ${mode}${opts.video ? '/--video' : ''} needs a dev --password; ${opts.video ? 'screenshots + video' : 'screenshots'} disabled for this run.`) + '\n');
  }
  ...
  const tasks = hosts.map((host) => {
    ...
    return runHost(host, password, plan.byHost.get(host), cfg, opts, c, write, multi).then((stats) => ({ stats, buf }));
  });
...
module.exports = { run, execStep, resolveTarget, pollUntil, parseHosts, buildPlan };
